...
var KiCAD = new function()
{
	...
	function Component(d)
	{
		...
		for(var t in d)
		{
			...
			case 'L': 
				...
				var ctor = storage.ctors["Part_" + CmpName] || storage.ctors['<unknown>'];
				res = new ctor();
				//Name = f[2];
				break;
			case 'F': 
	...
	function loadSch(data)
	{
		...
		for(x in Lines) items.push(Lines[x]);
		pts = null;
		ctl.update();
	};
	function loadLib(data)
	{
		var Lib = storage.data.partLib, obj, br, s;
			...
			if(s[0] == "DRAW")
			{
				...
				while(x < l && data[++x] !== "ENDDRAW")
				{
					...
					case 'P': // Линии P К-воТочек 0 1 толщина (x y) .. (x y) N
						...
						if(w === 0.0) w = defw;
						Draw.push("c.beginPath(); c.lineWidth = " + w.toString());
						var n = parseInt(s[1]);
						var A = parseInt(s[5]) * Km;
						var B = -parseInt(s[6]) * Km;
						SetB(A, B);
						Draw.push("c.moveTo(" + A.toString() + "," + B.toString() + ");");
						var X, Y, t = 7;
						while(--n)
						{
							X = parseInt(s[t++]) * Km;
							Y = -parseInt(s[t++]) * Km;
							SetB(X, Y);
							Draw.push("c.lineTo(" + X.toString() + "," + Y.toString() + ");");							
						}
						if((A == X && B == Y) || s[t] == "F") Draw.push("c.closePath();");
						if(s[t] == "F") Draw.push("c.fill();");
						Draw.push("c.stroke();");
					...
					case 'X': // Контакт: X имя номер x y длина направление ШрифтИмени ШрифтНомера 1 1 ElType GraphStyle
						...
						if(!L) break;//L = Km * 40;
						Draw.push("c.beginPath(); c.lineWidth = " + defw.toString());
						Draw.push("c.moveTo(" + X.toString() + "," + Y.toString() + ");");
						var Xn = X, Yn = Y;
						switch(s[6])
						{
					...
					case 'C': // Окружность C x y R
						...
						Draw.push("c.beginPath();");
			            Draw.push("c.arc(" + X.toString() + "," + Y.toString() + ", " + R.toString() + ", 0, 2 * Math.PI, false);");
						Draw.push("c.closePath(); c.stroke();");
						break;
					case 'A': // Дуга A x y R a1 a2 0 1 0 N x1 y1 x2 y2
						...
						SetB(X + R, Y + R); SetB(X - R, Y - R);
						Draw.push("c.beginPath();");
			            Draw.push("c.arc(" + X.toString() + "," + Y.toString() + ", " + R.toString() + ", " + a1 + "," + a2 + ", false);");
						Draw.push("c.stroke();");						
						break;
					case 'S': // Прямоугольник S X1 Y1 X2 Y2 0 1 0 F
						...
						if(w === 0.0) w = defw;
						Draw.push("c.lineWidth = " + w +";"); 
						if(s[8] == "F")
							Draw.push("c.fillRect(" + X + "," + Y + ", " + W + ", " + H +");");
						Draw.push("c.strokeRect(" + X + "," + Y + ", " + W + ", " + H +");"); 
						break;
					case 'T':// Текст T 0 x y size 0 0 0 текст вид 0 С С
						...
						var t = s[8];
			    		Draw.push("c.font = '" + S + "px monospace';");
			    		Draw.push("c.fillText('" + t + "'," + X + "," + Y + ");");						
						break;
					}
				}
				obj.D = new Function('c', Draw.join("\n"));
			}
